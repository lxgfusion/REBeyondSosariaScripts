"""
Deed-driven auto-tamer, with a butcher's round.
==============================================
...
Harvesting (merged in from Leatherman.py)
-----------------------------------------
TAMING TAKES PRIORITY. Every lap looks for a deed-backed creature first; only
when there is none in range - or no deed to fill, or no free follower slot -
does it look for something on HARVEST_WORDS to kill and butcher.

The round is: cast at it until it dies -> carve the corpse with a dagger ->
open the corpse -> say the loot command -> push what came out into the storage
keys. See the HARVEST_* config block.

A species on BOTH lists is KILLED, not tamed, and preflight says so at startup.
It cannot be both, and butchering something you meant to tame cannot be undone.

Scripts/Leatherman.py is the standalone version this came from. It stays in
place until this is confirmed in game; do not edit both.
"""
...
# From the catalogue below, which came out of ServUO source:
#     ("cow",  [0xD8, 0xE7], 11.1)
#     ("goat", [0xD1],       11.1)
...
def identify(mob):
    ...
    # A species on the harvest list is KILLED, never tamed - even if you are
    # holding a deed for it. It cannot be both, and of the two ways to be
    # wrong, taming something you meant to butcher costs a deed slot while
    # butchering something you meant to tame cannot be undone. preflight says
    # so out loud at startup rather than leaving it to be discovered.
    if HARVEST_ENABLED and is_harvest_target(name):
        debug("'%s' is on the harvest list - it gets killed, not tamed."
              % name, HUE_INFO)
        return None
...
def cast_at(serial, spell, school):
    ...
    THE one cast helper - defence and harvest both come through here. They had
    a copy each, and only one of the two copies got the cursor fix.
    ...
    # noshow is FALSE deliberately. True hides the cursor from the client, so
    # a cursor the server refuses - which is what happens when the last cast
    # lands as the creature dies - stays open where the player cannot see it,
    # and every target they click afterwards is silently eaten by it.
    ...
    # which case the server refuses the target and the cursor stays open.
    Misc.Pause(150)
    if Target.HasTarget():
...
def peacemake(serial, label):
    ...
    # noshow is FALSE everywhere in this script now. True hides the cursor
    # from the client, so one the server never consumes sits there unseen
    # and eats whatever the player clicks next. Nothing is gained by
    # hiding it, and an invisible leak cannot be diagnosed from in game.
    if not Target.WaitForTarget(TARGET_CURSOR_TIMEOUT, False):
        Misc.Pause(300)
        clear_cursor()
        return "failed"
...
def harvest_pass():
    ...
    # Anything closing on you is dealt with first. defend() is otherwise only
    # reached from inside a taming attempt, and a harvest round stands still
...
def preflight():
    ...
    if deeds == 0:
        ...
        # NOT fatal any more. With harvesting on there is still work to do, and
        # stopping here would mean an empty deed pack silently switched the
        # whole script off.
        if not harvest_preflight():
            return False
        log("Running in harvest-only mode - nothing will be tamed.", HUE_WARN)
        return True

    log("Taming: %s" % ", ".join(sorted(_active)), HUE_GOOD)
    harvest_preflight()
    return True


def harvest_preflight():
    """Report the harvest half. False when it cannot run at all.

    Every piece it needs is named and checked here rather than discovered in
    the field: a missing dagger means corpses are never carved and a missing
    storage key means the pack fills and stays full - both of which look, from
    in game, like the script simply stopped working.
    """
    if not HARVEST_ENABLED:
        log("Harvesting: off (HARVEST_ENABLED is False).", HUE_INFO)
        return False
    if not HARVEST_WORDS or not HARVEST_BODIES:
        log("Harvesting: on, but HARVEST_WORDS or HARVEST_BODIES is empty, so "
            "nothing can be found.", HUE_BAD)
        return False
    if not HARVEST_SPELL:
        log("Harvesting: on, but HARVEST_SPELL is empty and there is no "
            "fallback - set it to the spell you want cast.", HUE_BAD)
        return False

    log("Harvesting: %s" % ", ".join(HARVEST_WORDS), HUE_GOOD)
    if HARVEST_NEVER_WORDS:
        log("  never: %s" % ", ".join(HARVEST_NEVER_WORDS), HUE_INFO)
    log("  %s, carve with 0x%04X, then say %r."
        % (HARVEST_SPELL, HARVEST_DAGGER_ID, HARVEST_GRAB_PHRASE), HUE_INFO)

    # Say it plainly when a species is on both lists. It cannot be both, the
    # kill wins, and finding that out by watching a deed species get butchered
    # is the worst possible way to learn it.
    clash = sorted(name for name in _active if is_harvest_target(name))
    if clash:
        log("  %s is on BOTH lists - it will be KILLED, not tamed. Take it out "
            "of HARVEST_WORDS if you want it tamed." % ", ".join(clash),
            HUE_WARN)
...
def main():
    ...
    try:
        ...
        # However this ends - the Stop button, an exception, the player dying -
        # do not walk away holding a target cursor. One left open is answered
        # by whatever the player clicks next.
        try:
            Target.Cancel()
            Target.ClearQueue()
...
def harvest_fallback():
    """Harvest, because there is no taming to do. True if it did something.

    TAMING TAKES PRIORITY: every caller has already established that there is
    no tameable candidate in range, no deed to fill, or no follower slot to put
    one in. This is what the script does with the time instead.
    """
    if not HARVEST_ENABLED:
        return False
    return harvest_pass()
...
def run():
    ...
    def quiet(text, hue=HUE_WARN):
        """Say it at most once a minute. These fire every lap of the loop."""
        if time.time() - idle_notice[0] >= 60.0:
            idle_notice[0] = time.time()
            log(text, hue)
    ...
    while True:
        ...
        if not _active:
            # No deed to fill. Harvesting is the only work there is.
            if harvest_fallback():
                continue
            quiet("No taming order deeds. Harvesting only.", HUE_INFO)
            Misc.Pause(IDLE_PAUSE * 5)
            rescan("waiting for deeds")
            continue

        if Player.Followers >= Player.FollowersMax:
            # Nothing can be tamed until a slot frees up. A cow needs none.
            if harvest_fallback():
                continue
            quiet("Follower slots are full. Harvesting while you wait.")
            Misc.Pause(IDLE_PAUSE * 5)
            continue

        candidates = find_candidates()
        if not candidates:
            # THE fallback that matters: taming had first refusal and found
            # nothing in range.
            if harvest_fallback():
                continue
