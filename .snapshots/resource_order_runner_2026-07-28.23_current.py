...
Nothing is held back - KEEP_PER_TYPE is 0. A single resource can still be
protected by giving its RESOURCES entry a "keep": N.

Each lap works only the resources actually IN STOCK, one at a time and to
exhaustion, rotated so the next lap resumes where the last stopped. Without that
filter the withdrawal cap gets spent on names the book wants but the chests do
not hold.
    ...
    2. Work out a budget per resource, and keep only the ones with stock.
    3. For each resource, filter the order book to its exact name and scan
       EVERY page of the result, taking the biggest order that fits. Resources
       are cycled round-robin - one order each per pass - so anything fillable
       gets a deed early rather than waiting behind a resource with deep stock.
       Each lap RESUMES where the last stopped.
    ...
    7. Back to Start Fill and round again. When a full pass over every stocked
       resource fills nothing, visit FINAL_STATIONS once to clear the last
       lap's rewards, and stop.
...
# Ceiling on laps. This is a SAFETY NET, not the intended stop - the run is
# meant to end when a full pass fills nothing, having emptied what it can.
# At MAX_ORDERS_PER_RUN a lap, 20 laps capped the whole run at 500 orders,
# which is short of what the chests can actually fill.
MAX_CYCLES = 200
...
# Visited ONCE when the run ends, after the last lap. The rewards from the final
# lap are still in the pack at that point, so this is what clears them before
# the script stops. Names must match a STATIONS entry's "label"; [] to skip.
FINAL_STATIONS = ["Deposit items"]

# Where to go when the whole run ends, after FINAL_STATIONS. Empty = stay put.
RETURN_FOLDER = []
RETURN_POINT = ""
...
# Left in the chest for every resource. ZERO - everything is spendable.
#
# Was 100 while it mattered; it no longer applies to anything. The mechanism
# stays because it costs nothing and a resource can still be protected on its
# own by giving its RESOURCES entry a "keep": N.
KEEP_PER_TYPE = 0
...
# EVERY page of a filtered result is scanned - the page counter at the bottom of
# the gump says how many there are, and the scan runs to the last one.
#
# This is only a safety cap. A filter narrows 540 pages down to at most about
# thirteen (Agapite Granite, the busiest, has 179 orders at 15 a page), so it
# should never be reached.
#
# Scanning a fixed few pages was what hid Copper Ingots: the Name filter is a
# SUBSTRING match, so "Copper Ingots" also returns all 101 "Dull Copper Ingots"
# rows. Those are rejected correctly, but they fill the early pages, and a
# four-page scan reported "no Copper Ingots orders" with 31 of them waiting
# further in.
MAX_PAGES_PER_RESOURCE = 40
...
# Nothing is held back any more - KEEP_PER_TYPE is 0 for everything. To protect
# one resource, give its RESOURCES entry a "keep": N.
...
# How long to wait for the order list's CONTENT to change after a button press,
# and how often to look. See orders_action - waiting on the gump id alone comes
# back instantly with the previous page still loaded.
GUMP_CHANGE_TIMEOUT_MS = 4000
GUMP_POLL_MS = 120
...
def gump_fingerprint():
    """Something about the order list that changes when it is replaced."""
    try:
    ...
    if filter_text is None:
        ...
        try:
            ...
            log("SendAdvancedAction failed (%s); the filter will be lost." % err,
                HUE_WARN)
            Gumps.SendAction(ORDERS_GUMP, button)
    # WAIT FOR THE CONTENT TO CHANGE, not for a gump to exist.
    #
    # Gumps.WaitForGump returns True immediately for a gump that is ALREADY
    # open, and the server answers this button by replacing the list with a NEW
    # gump under the SAME id. So waiting on the id alone returns at once and the
    # next read hands back the OLD page.
    #
    # When that happened after a filter, the rows read were the unfiltered ones:
    # the anchor matched none of them, rows and buttons disagreed, the page was
    # skipped, and the resource was reported as having no orders. Timing
    # dependent, which is why it struck some resources and not others.
    deadline = time.time() + (GUMP_CHANGE_TIMEOUT_MS / 1000.0)
    while time.time() < deadline:
        Misc.Pause(GUMP_POLL_MS)
        if not has_gump(ORDERS_GUMP):
            return False
        if gump_fingerprint() != before:
            return True

    if has_gump(ORDERS_GUMP):
        log("the list did not change within %dms of pressing %d%s - reading it "
            "as it stands." % (GUMP_CHANGE_TIMEOUT_MS, button,
                               " (filter %r)" % filter_text
                               if filter_text else ""), HUE_WARN)
    return has_gump(ORDERS_GUMP)


def read_page(tries=4):
    """(strings, layout) for the order list, as ONE consistent snapshot.

    The two come from separate Razor calls. If a new page lands between them,
    the rows are from one page and the buttons from another - and because both
    pages carry 15 rows, the row/button count check passes and the wrong order
    gets pressed. The fingerprint is taken either side and the read repeated if
    it moved.
    """
    for attempt in range(max(1, tries)):
        ...
        log("the list changed mid-read - retrying (%d)" % (attempt + 1),
            HUE_WARN)
        Misc.Pause(GUMP_POLL_MS)
    log("the list would not hold still long enough to read it.", HUE_BAD)
    return [], ""
...
def find_first_order(resource, budget):
    ...
    if collisions:
        log("%r also matches %s, whose rows will fill some of these pages"
            % (term, ", ".join(collisions)))

    rejected = 0
    for page in range(1, MAX_PAGES_PER_RESOURCE + 1):
        strings, layout = read_page()
        ...
        if len(rows) != len(buttons):
            ...
            # Zero rows against a full set of buttons means the page holds
            # somebody else's orders - i.e. the filter had not taken effect
            # when this was read. Naming what IS on it makes that obvious.
            if not rows and buttons:
                loose = parse_order_rows(strings)
                log("  the page actually holds: %s"
                    % (", ".join(sorted(set(r["name"] for r in loose))[:6])
                       or "nothing readable"), HUE_BAD)
                log("  -> the %r filter was not applied to what was read."
                    % term, HUE_BAD)
        else:
            # The BIGGEST order that fits, not the first. The goal is to empty
            # the chest, and a bigger order spends more stock for the same
            # withdrawal, the same deed and the same trip to the hand-in.
            #
            # Only ever chosen from THIS page, because the button is pressed
            # while the list is still on it - going back to a better one seen
            # earlier means pressing a remembered button on a live list.
            best = None
            for row, button in zip(rows, buttons):
                ...
                if best is None or amount > best["amount"]:
                    best = {"button": button, "name": row["name"],
        ...
        if page >= MAX_PAGES_PER_RESOURCE:
            log("gave up after %d of %d pages for %r - raise "
                "MAX_PAGES_PER_RESOURCE" % (page, total, term), HUE_WARN)
            break
        if not orders_action(ORDERS_NEXT_BUTTON, term):
            break

    if rejected:
        log("%r: nothing fits - %d row(s) over %d page(s) belonged to another "
            "resource" % (term, rejected, page))
    return None
...
def visit_station(station):
    ...
    return use_context_item(item, station.get("context", []), label)
...
def fill_orders(chests, offset=0):
    ...
    # Only resources with stock go in the lap order. The table holds all 79
    # names the book uses, ordered by how many orders it has for each - and
    # Copper Ingots sits at position 57 because the book only wants 31 of them.
    # Walking the full table spends the withdrawal cap on resources there is
    # nothing to fill with, and a chest holding 34,000 copper never gets a turn.
    stocked = [r for r in worked_resources() if budget.get(r["name"], 0) > 0]
    if not stocked:
        log("nothing in stock matches anything the book asks for.", HUE_WARN)
        return [], False, 0

    order = rotated(stocked, offset)
    log("%d resource(s) in stock this lap, starting at %s"
        % (len(stocked), order[0]["name"]))
    if len(stocked) > MAX_ORDERS_PER_RUN:
        laps_needed = (len(stocked) + MAX_ORDERS_PER_RUN - 1) // MAX_ORDERS_PER_RUN
        log("more in stock than MAX_ORDERS_PER_RUN (%d), so a full pass over "
            "them takes %d lap(s) - raise it to %d to reach every resource "
            "every lap." % (MAX_ORDERS_PER_RUN, laps_needed, len(stocked)),
            HUE_WARN)

    # ROUND ROBIN: one order per resource per pass, cycling until a whole pass
    # takes nothing or the lap cap is reached.
    #
    # Depth-first let one resource with deep stock swallow the lap, so anything
    # behind it waited for the next one. Cycling means every resource that has
    # something fillable gets a deed early, which is the point - pull ANY deed
    # that can be filled.
    for _round in range(1, MAX_ORDERS_PER_RUN + 1):
        ...
        for entry_def in order:
            ...
            if outcome == "none":
                log("%s: nothing more that fits" % resource)
                exhausted.add(resource)
                continue
            took_any = True

        if not took_any:
            log("a full pass took nothing - every resource is done for now")
            break
    ...
    # Wrapped against the stocked list, which is what `order` was built from.
    return completed, False, (offset + len(examined)) % max(1, len(stocked))
...
def main():
    ...
    # Nothing left to fill - clear the last lap's rewards before stopping.
    for label in FINAL_STATIONS:
        station = None
        for candidate in STATIONS:
            if candidate.get("label") == label:
                ...
                break
        if station is None:
            log("FINAL_STATIONS names %r, which is not a STATIONS entry."
                % label, HUE_BAD)
            continue
        visit_station(station)
